...
## 4. Accès administrateur (ajout de questions)

Un mot de passe protège l'ajout/la suppression de questions.
- Mot de passe par défaut : **Craie2026**
- Pour le changer : ouvre `index.html`, cherche la ligne
  `var ADMIN_PASSWORD = "Craie2026";` tout en haut du script, remplace le texte
  entre guillemets, enregistre, puis remets le fichier en ligne sur GitHub.
- ⚠️ Comme le site est 100% statique (pas de serveur), ce mot de passe est
  visible dans le code source par quelqu'un de très curieux qui saurait
  regarder le code de la page. C'est une protection simple pour éviter que
  n'importe quel joueur modifie les questions par erreur ou par curiosité,
  pas une sécurité de niveau bancaire.
- Une fois le bon mot de passe entré, il reste valide tant que l'appli n'est
  pas fermée/rechargée ; il est redemandé à chaque nouveau lancement.

## 5. Sons du jeu

- ✅ Bonne réponse : une petite mélodie + une voix qui dit "Bonne réponse !"
- ❌ Mauvaise réponse : une mélodie + une voix qui dit "Mauvaise réponse !"
- 🎉 Niveau terminé sans une seule erreur : un son d'applaudissements + "Bravo !
  Niveau réussi sans faute !"
- Un bouton 🔊/🔇 en haut à droite permet de couper le son (préférence gardée
  d'une visite à l'autre).
- Ces sons sont générés directement par le téléphone (voix de synthèse du
  navigateur + Web Audio), donc pas besoin de connexion internet ni de
  fichiers audio séparés. La voix française dépend des voix installées sur
  l'appareil : elle est présente par défaut sur la plupart des téléphones
  Android et iPhone récents.
